...
if (!class_exists('decideprice_theme')){require_once('decideprice_theme.php');}
	...
	if ( $type !== '' )
	{
		...
		if ( $type === 'update_cart' )
		{
			...
			if ( isset( $_COOKIE[ 'decideprice_cart' ] ) )
			{
				...
				if ( is_array( $cart_data ) )
				{
					...
					{
						// print_r($value['merchant_user_id']);
						$cnt = 1;
						...
						foreach ( $cart_count as $key1 => $value1 )
						{
							...
							if ( isset( $value1[ $value[ 'merchant_user_id' ] . '_' . $value[ 'shop_id' ] . '_' . $value[ 'prod_det_id' ] ] ) )
							{
								$cnt = $value1[ $value[ 'merchant_user_id' ] . '_' . $value[ 'shop_id' ] . '_' . $value[ 'prod_det_id' ] ];
							}
						}
		...
		if ($type === 'update_item_qty')
...

			{
				...
				echo json_encode( array( 'msg' => 'set' ) );
			}
